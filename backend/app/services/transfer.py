...
class TransferService:
    ...
    def get_by_id(self, transfer_id: UUID, user_id: UUID) -> Transfer:
        """
        ...
        Raises:
            NotFoundError: If transfer not found or not owned by this user.
        """
        transfer = self.repository.get_with_relations(transfer_id, user_id)
        if not transfer:
    ...
    def create(
        ...
        Args:
            ...
            base_rate: Units of primaryCurrency per 1 unit of the source
                account's native currency at transfer time.
        ...
        source_account = self.account_repository.get_by_id_or_fail(
            source_account_id, user_id
        )
        destination_account = self.account_repository.get_by_id_or_fail(
        ...
        if cross_currency:
            if destination_amount is None or exchange_rate is None:
                raise ValidationError(
                    "destination_amount and exchange_rate are required for "
                    "cross-currency transfers"
                )
    ...
    def update(
        ...
        """
        transfer = self.repository.get_by_id_or_fail(transfer_id, user_id)
