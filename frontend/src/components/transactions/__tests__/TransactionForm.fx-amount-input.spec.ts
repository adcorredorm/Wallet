...
describe('TransactionForm — FX section uses AmountInput', () => {
  it('renders AmountInput for originalAmount in FX section (not input[type="number"])', async () => {
    ...
    // Confirm no input[type="number"] exists for the amount fields.
    // NOTE: the exchange_rate field remains a plain input[type="number"] until Task 4.
    // After this refactor, originalAmount and accountAmount use AmountInput (type="text"
    // inputmode="decimal"), so the count of type="number" inputs should be exactly 1
    // (only the exchange_rate field).
    const numberInputs = wrapper.findAll('input[type="number"]')
    expect(numberInputs.length).toBe(1) // only exchange_rate remains
  })
