...
import type { PendingMutation, LocalAccount, LocalTransaction, LocalTransfer, LocalCategory, LocalDashboard, LocalDashboardWidget, LocalExchangeRate, LocalRecurringRule, LocalBudget } from './types'
...
export const DEPENDENCY_FIELDS: Record<PendingMutation['entity_type'], string[]> = {
  ...
  // recurring_rule has no FK dependencies from other entities.
  // Transactions reference rules via recurring_rule_id, but transactions
  // generated by the engine are standalone — they don't fail if the rule fails.
  recurring_rule: [],
  // budget has no FK dependencies from other entities.
  budget: [],
}
...
export class SyncManager {
  ...
  private async findServerId(tempId: string): Promise<string | undefined> {
    ...
    const budget = await db.budgets.get(tempId)
    if (budget?.server_id) return budget.server_id

    const budgetByOfflineId = await db.budgets.filter(b => b.offline_id === tempId).first()
    if (budgetByOfflineId?.server_id) return budgetByOfflineId.server_id
  ...
  private async resolveTemporaryId(
    ...
    switch (entityType) {
      ...
      case 'budget':
        // Budget does not act as a FK in any other Dexie table.
        break
    }
  ...
  private async replaceEntityWithRealId(
    ...
    switch (entityType) {
      ...
      case 'budget': {
        const old = await db.budgets.get(tempId)
        if (old) {
          ...
          await db.budgets.delete(tempId)
  ...
  private async markSynced(
    ...
    switch (entityType) {
      ...
      case 'budget':
        await db.budgets.update(serverResult.id, syncFields)
        break
    }
  }
  ...
  private async markEntitySyncStatus(
    ...
    switch (entityType) {
      ...
      case 'budget': await db.budgets.update(entityId, fields); break
    }
  }
  ...
  private async markError(
    ...
    switch (entityType) {
      ...
      case 'budget':
        await db.budgets.update(entityId, errorFields)
        break
    }
  }
  ...
  private async fullReadSync(): Promise<void> {
    ...
    const results = await Promise.allSettled([
      ...
      syncEntity<LocalBudget>(
        '/budgets?limit=10000',
        'budgets',
        (items) => db.budgets.bulkPut(items.map((item) => toLocalItem(item, item.id) as LocalBudget)),
      ),
      this.syncDashboards(),
    ])

    const names = ['accounts', 'transactions', 'transfers', 'categories', 'exchange_rates', 'recurring_rules', 'budgets', 'dashboards']
  ...
  private async incrementalSync(): Promise<void> {
    ...
    const entities: EntityConfig[] = [
      ...
      },
      {
        url: '/budgets',
        cursorKey: 'budgets',
        writer: (items) => db.budgets.bulkPut(items.map((i) => toLocalItem(i, i.id) as LocalBudget)),
      },
    ]
